...
/**
 * A GeoResource implementation for {@link H2GISSchemaFolder}.  Resolves to a Featurestore
 *
 * @author Erwan Bocher, CNRS 2021
 * @author Jesse Eichar, Refractions Research
 * @since 1,2
 */
public class H2GISGeoResource2 extends IGeoResource {
    final TableDescriptor desc;
    final String typename;
    private volatile Status status;
    private volatile Throwable message;
    private final URL identifier;
    private final H2GISSchemaFolder parent;
    private Boolean readOnly = null; // we won't know until we try
    
    public H2GISGeoResource2( H2GISService2 service, H2GISSchemaFolder h2gisSchemaFolder, TableDescriptor desc ) {        
            this.service = service;
            this.parent=h2gisSchemaFolder;
            this.desc = desc;
    ...
    protected H2GISResourceInfo createInfo( IProgressMonitor monitor ) throws IOException {
        try {
            return new H2GISResourceInfo(this);
        } catch (Exception e) {
            H2GISPlugin.log("Error creating a H2GISInfo object", e);
            return null;
        }      
    }
