...
import static org.locationtech.udig.catalog.H2GISServiceExtension2.DIALECT;
...
import org.locationtech.udig.catalog.H2GISGeoResource2;
import org.locationtech.udig.catalog.H2GISSchemaFolder;
import org.locationtech.udig.catalog.H2GISService2;
...
public class H2GISConnectionFactory extends UDIGConnectionFactory {
    ...
    public Map<String, Serializable> createConnectionParameters(Object context) {
		if( context instanceof H2GISService2 ){
            H2GISService2 h2gis = (H2GISService2) context;
            return h2gis.getConnectionParams();
        }
        URL url = toCapabilitiesURL( context );
        if( url == null ){
            // so we are not sure it is a H2GIS url
            // lets guess
            ID sample = ID.cast( context );
            url = sample != null ? sample.toURL() : null;
        }
        if( url != null && H2GISServiceExtension2.isH2GIS(url)) {  
            ...
            for( IResolve resolve : list ){
                if( resolve instanceof H2GISService2) {
                    // got a hit!
                    H2GISService2 h2gisService = (H2GISService2) context;
                    return h2gisService.getConnectionParams();
                }
                else if (resolve instanceof H2GISSchemaFolder) {
                    H2GISSchemaFolder h2gisFolder = (H2GISSchemaFolder) resolve;
                    return h2gisFolder.getService(new NullProgressMonitor()).getConnectionParams();
                }
                else if (resolve instanceof H2GISGeoResource2 ){
                    H2GISGeoResource2 layer = (H2GISGeoResource2) resolve;
                    H2GISService2 h2gis;
                    try {
                        h2gis = (H2GISService2) layer.parent( null );
                        return h2gis.getConnectionParams();
                    } catch (IOException e) {
	...
	/**
     * Convert "data" to a H2GIS url
     * <p>
     * Candidates for conversion are:
     * <ul>
     * <li>URL - from browser DnD
     * <li>H2GISService2 (the uDig class) - from catalog DnD
     ...
     * @return URL considered a possibility for a H2GIS connection, or null
     */
    ...
    protected URL toCapabilitiesURL( IService resolve ){
        if( resolve instanceof H2GISService2 ){
            return toCapabilitiesURL( (H2GISService2) resolve );
        }
        return toCapabilitiesURL( resolve.getIdentifier() );        
    }
    /** No further QA checks needed - we know this one works */
    protected URL toCapabilitiesURL( H2GISService2 h2gis ){
        return h2gis.getIdentifier();                
    }
    /** Quick sanity check to see if url is a H2GIS url */
    ...
    /** Quick sanity check to see if url is a H2GIS url String */
    protected URL toCapabilitiesURL( String string ){
        if (string == null) return null;

        if( !string.contains(DIALECT.urlPrefix) && !string.contains("jdbc.h2gis") && !string.contains("h2gis") ) { //$NON-NLS-1$ //$NON-NLS-2$ 
            return null;
        }
        ...
        try {
            ...
            // TODO Catch e
            H2GISPlugin.log("bad url", e); //$NON-NLS-1$
        }
        return url;
    }
    ...
    protected Map<String,Serializable> createParams( URL url ){
        H2GISServiceExtension2 serviceFactory = new H2GISServiceExtension2();
        @SuppressWarnings("rawtypes")
