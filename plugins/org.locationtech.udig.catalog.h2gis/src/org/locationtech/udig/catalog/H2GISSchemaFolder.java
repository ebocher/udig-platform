...
/**
 * A Folder that represents a schema in a H2GIS folder. Its members are the tables and are
 * featurestores. This resolves to a H2GISDataStore
 * 
 * @author jesse
 * @since 1.1.0
 */
public class H2GISSchemaFolder implements IResolveFolder {

    private final String schema;
    private final ArrayList<IResolve> members;
    private final H2GISService2 service;
    private final URL identifier;
    private Exception trace;
	private HashMap<String, Serializable> params;
	private JDBCDataStore datastore;

    public H2GISSchemaFolder( H2GISService2 service, String schema, Collection<TableDescriptor> descriptors) {
        ...
        for (TableDescriptor tableDescriptor : descriptors) {
            H2GISGeoResource2 resource2 = new H2GISGeoResource2(service, this, tableDescriptor);
    ...
    protected void finalize() throws Throwable {
        if( trace!=null ){
            // uh oh not disposed!
            H2GISPlugin.log("H2GISSchemaFolder was not disposed!", trace); //$NON-NLS-1$
    ...
    public synchronized JDBCDataStore getDataStore() throws IOException {
    	if(datastore == null) {
    		H2GISDataStoreFactory factory = H2GISServiceExtension2.getFactory();
